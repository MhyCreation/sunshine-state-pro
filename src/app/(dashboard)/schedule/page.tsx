...
export default async function SchedulePage({
  ...
  weekEnd.setDate(weekStart.getDate() + 13); // two weeks out
    ...
    <div className="max-w-7xl mx-auto">
      <header className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-navy-500 mt-1">{jobs.length} jobs in the next 2 weeks</p>
        </div>
        <Link
      ...
      {jobs.length === 0 ? (
        <div className="bg-white rounded-lg border border-navy-100 flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-navy-400 mt-1">Create your first job to fill the schedule.</p>
        </div>
      ) : (
            ...
            return (
              <div key={dateKey}>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-xs text-navy-400">{grouped[dateKey].length} job{grouped[dateKey].length !== 1 ? "s" : ""}</span>
                </div>
