...
// Build a parameterized WHERE clause from a flat filter object.
// Supports operators via dot notation: "column.gt", "column.like", etc.
...
export function createRestRoutes(db: DB, config: LBConfig, emit: EmitFn): Hono {
  const app = new Hono()
  ...
  app.get('/:table', async (c) => {
    ...
    for (const [k, v] of Object.entries(qs)) {
      if (!RESERVED.has(k)) userFilters[k] = v
    }
    ...
    const sql = `SELECT ${cols} FROM "${table}" ${where} ${order} LIMIT ? OFFSET ?`
    ...
    try {
      ...
      const { count } = db.queryOne<{ count: number }>(
        `SELECT COUNT(*) as count FROM "${table}" ${countWhere}`,
        countParams
      ) ?? { count: 0 }
      return c.json({ data: rows, count })
    } catch (err: any) {
      return c.json({ error: { code: 'QUERY_ERROR', message: err.message } }, 400)
    }
  })
  ...
  app.get('/:table/:id', async (c) => {
    ...
    try {
      const row = db.queryOne(`SELECT * FROM "${table}" WHERE id = ? ${extra}`, [...params, ...rlsParams])
  ...
  app.post('/:table', async (c) => {
    ...
    try {
      ...
      return c.json({ error: { code: 'INSERT_ERROR', message: err.message } }, 400)
    }
  })
  ...
  app.patch('/:table/:id', async (c) => {
    ...
    try {
      ...
      return c.json({ error: { code: 'UPDATE_ERROR', message: err.message } }, 400)
    }
  })
  ...
  app.delete('/:table/:id', async (c) => {
    ...
    try {
      ...
      return c.json({ error: { code: 'DELETE_ERROR', message: err.message } }, 400)
    }
  })
