...
const REFRESH_TOKEN_TTL_SECS = 30 * 24 * 3600
...
export async function verifyAccessToken(token: string, config: LBConfig): Promise<AuthUser | null> {
  try {
    ...
    return { id: payload.sub!, email: payload['email'] as string, role: payload['role'] as string, metadata: {}, created_at: '' }
  } catch { return null }
}

type UserRow = { id: string; email: string; password_hash: string; role: string; metadata: string; created_at: string }

function rowToUser(row: UserRow): AuthUser {
  return { id: row.id, email: row.email, role: row.role, metadata: JSON.parse(row.metadata), created_at: row.created_at }
}
...
function newRefreshSession(db: DB, userId: string): string {
  ...
  db.run('INSERT INTO auth_sessions (id, user_id, refresh_token, expires_at) VALUES (?, ?, ?, ?)',
    [crypto.randomUUID(), userId, refreshToken, expiresAt])
  return refreshToken
}

// getDb receives the Hono Context so it can resolve the per-project DB for this request
export function createAuthRoutes(getDb: (c: Context) => DB, config: LBConfig): Hono {
  const app = new Hono()
  ...
  app.post('/refresh', async (c) => {
    ...
    const session = db.queryOne<{ id: string; user_id: string; expires_at: string }>(
      'SELECT * FROM auth_sessions WHERE refresh_token = ?', [body.refresh_token])
    ...
    if (!row) return c.json({ error: { code: 'USER_NOT_FOUND', message: 'user not found' } }, 404)
    const user = rowToUser(row)
